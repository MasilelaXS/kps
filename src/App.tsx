...
import { PendingReportsScreen } from './pages/mobile/reports/PendingReportsScreen';
...
function App() {
  ...
  useEffect(() => {
    if (user?.id) {
      const cleanup = SyncService.setupAutoSync(user.id);
      return cleanup;
    }
  }, [user?.id]);
  ...
  return (
    <ErrorBoundary>
                ...
                <Route path="/*" element={
                  <MobileLayout>
                    <Routes>
                      ...
                      <Route path="reports/pending" element={<PendingReportsScreen />} />
      ...
      <Toaster
        position="top-right"
        containerStyle={{
          top: '80px', // Move below topbar (assuming topbar is ~64px + padding)
        }}
        toastOptions={{
          duration: 4000,
          style: {
