...
import { ArrowLeft, PenTool, RotateCcw, Check, WifiOff, Cloud } from 'lucide-react';
...
export const SignatureScreen: React.FC = () => {
  ...
  if (showSuccess) {
    const isOffline = submitResult?.isOffline || false;
    
    return (
      <div className={`min-h-screen ${isOffline ? 'bg-gradient-to-br from-blue-50 to-blue-100' : 'bg-gradient-to-br from-green-50 to-green-100'} flex items-center justify-center p-4`}>
        <div className="max-w-md w-full bg-white rounded-lg shadow-xl p-6 text-center">
          {/* Success Icon */}
          <div className={`w-16 h-16 ${isOffline ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'} rounded-full flex items-center justify-center mx-auto mb-4`}>
            {isOffline ? <Cloud className="w-8 h-8" /> : <Check className="w-8 h-8" />}
          </div>
          ...
          <p className="text-sm text-gray-600 mb-6">
            {isOffline 
              ? 'No internet connection detected. Your report has been saved locally and will sync automatically when connection is restored.'
          ...
          {isOffline && (
            <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-4 flex items-center space-x-2">
              <WifiOff className="h-4 w-4 text-blue-600" />
              <span className="text-sm text-blue-800">Will sync when online</span>
            </div>
          )}
          ...
          <div className="bg-gray-50 rounded p-3 mb-6">
            <div className="grid grid-cols-1 gap-2 text-xs">
              <div className="flex justify-between">
                ...
                <span className={`font-bold ${isOffline ? 'text-blue-600' : 'text-green-600'}`}>
                  {isOffline ? 'OFFLINE-PENDING' : (currentReport?.report_id || 'AUTO-GENERATED')}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-medium ${isOffline ? 'text-blue-600' : 'text-green-600'}`}>
                  {isOffline ? 'Saved Offline' : 'Submitted Online'}
                </span>
              </div>
            </div>
          </div>
