...
export const ClientsPage: React.FC = () => {
  ...
  const handleUnassignClick = async (client: Client) => {
    ...
    try {
      ...
      // Use the API to unassign the client from all PCOs
      const response = await adminService.assignClientsToPCO({
        client_ids: [client.id],
        action: 'unassign'
        // pco_id is omitted to remove all assignments for the client
      });
  ...
  const handleAssignSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await adminService.assignClientsToPCO({
        ...
        assigned_by: user?.id // Use current user's ID, fallback to 1 if not available
      });
  ...
  const handleUpdateNotes = async () => {
    ...
    try {
      ...
      const response = await adminService.createClientNote({
        ...
        user_id: user?.id || 1, // Use current user's ID, fallback to 1 if not available
