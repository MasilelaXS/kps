...
interface MobileStoreActions {
  ...
  // Report submission
  submitReport: () => Promise<{ success: boolean; message?: string; reportId?: number; isOffline?: boolean; id?: string | number }>;
    ...
    (set, get) => ({
      ...
      submitReport: async () => {
        ...
        try {
          ...
          if (!isConnected) {
            // Save to offline storage - save the complete ReportSubmission data
            const userId = useAuthStore.getState().user?.id || 1;
            
            const offlineReportId = OfflineStorageService.saveOfflineReport(submissionData, userId);
          ...
          // Check for potential duplicates before submitting online
          const duplicateCheck = await SyncService.checkForDuplicates(
            submissionData.client_id,
            submissionData.date_of_service,
            submissionData.report_type
          );
          ...
          if (result.success) {
            ...
            return { 
              success: true, 
              isOffline: false, 
              id: result.report_id || generatedReportId, 
              reportId: result.report_id || generatedReportId,
          ...
          if (current) {
            ...
            // Generate a 6-digit report ID for the offline backup
            const generateReportId = (): number => {
            ...
            // Recreate the full ReportSubmission object for offline storage
            const backupSubmissionData: ReportSubmission = {
              report_id: generatedReportId,
              client_id: current.client_id!,
              report_type: current.report_type!,
              date_of_service: formatDateForMySQL(current.date_of_service!),
              ...
              warning_signs_quantity: current.inspection_remarks?.warning_signs_qty || 0,
              recommendations: current.recommendations || current.inspection_remarks?.inspection_notes || '',
              next_service_date: formatDateForMySQL(current.next_service_date || '', true),
              client_name: current.client_signature?.client_name || '',
              client_signature: current.client_signature?.signature_data || '',
              ...
              stations: (current.inspection_stations || []).map(station => ({
                ...
                access_reason: station.access_reason || '',
                has_activity: station.has_activity ? 1 : 0,
                activity_type: station.has_activity && station.activity_type ? station.activity_type : 'droppings',
                activity_type_other: station.activity_type_other || '',
                activity_description: station.activity_description || '',
                station_condition: station.station_condition || ['good'],
                bait_status: station.bait_status || 'eaten',
                rodent_box_replaced: station.rodent_box_replaced ? 1 : 0,
                poison_used_id: station.poison_used_id || 0,
                poison_quantity: station.poison_quantity || 0,
            ...
            const offlineReportId = OfflineStorageService.saveOfflineReport(backupSubmissionData, userId);
          ...
          // Only throw error if we couldn't save offline either
          set({ 
